...
exports.expandShortUrl = void 0;
const firebase_functions_1 = require("firebase-functions");
const node_fetch_1 = require("node-fetch");
const cors = require("cors");
const corsHandler = cors({ origin: true });
exports.expandShortUrl = firebase_functions_1.https.onCall(async (data, context) => {
    try {
        const { url } = data;
        if (!url) {
            throw new Error('URL is required');
        }
        firebase_functions_1.logger.info('Expanding short URL:', url);
            ...
            try {
                ...
                if (response.status >= 300 && response.status < 400) {
                    ...
                    if (location) {
                        ...
                        firebase_functions_1.logger.info('Expanded URL found:', expandedUrl);
                    }
                }
                else {
                    // リダイレクトがない場合、GETリクエストでHTMLを取得してパース
                    const htmlResponse = await (0, node_fetch_1.default)(url, {
                        headers: {
                            'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36'
                        }
                    });
                ...
                firebase_functions_1.logger.error('Failed to expand short URL:', fetchError);
                // フォールバック: URLパターンから推測
        ...
        firebase_functions_1.logger.error('Error in expandShortUrl:', error);
        return {
            success: false,
...
function extractCoordinatesFromUrl(url) {
    try {
        ...
        firebase_functions_1.logger.error('Error extracting coordinates:', error);
        return null;
    }
}
//# sourceMappingURL=expandShortUrl.js.map