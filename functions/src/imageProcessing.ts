 ...
 * 店舗画像の処理
 */
export const processShopImage = functions.storage.object().onFinalize(async (object) => {
  const filePath = object.name;
  const contentType = object.contentType;

  // 店舗画像のパスかチェック
  if (!filePath?.startsWith('shops/') || !contentType || !SUPPORTED_FORMATS.includes(contentType)) {
    return;
  }
  ...
  try {
    ...
    if (shopId && resizedUrls.length > 0) {
      ...
      const existingImages = shopDoc.data()?.images || [];
 ...
 * イベント画像の処理
 */
export const processEventImage = functions.storage.object().onFinalize(async (object) => {
  const filePath = object.name;
  const contentType = object.contentType;

  // イベント画像のパスかチェック
  if (!filePath?.startsWith('events/') || !contentType || !SUPPORTED_FORMATS.includes(contentType)) {
    return;
  }
  ...
  try {
    ...
    if (eventId && resizedUrls.length > 0) {
      ...
      const existingImages = eventDoc.data()?.images || [];
 ...
 * 画像制限チェック（5枚制限の実装）
 */
export const enforceImageLimit = functions.storage.object().onFinalize(async (object) => {
  const filePath = object.name;
  
  if (!filePath?.startsWith('shops/') && !filePath?.startsWith('events/')) {
    return;
  }
